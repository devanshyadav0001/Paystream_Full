...
export function DonutChart({ data, size = 180, thickness = 30, title }) {
    ...
    return (
        <div className="flex flex-col items-center">
            {title && <div className="text-xs font-semibold mb-3 uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>{title}</div>}
            <div className="relative" style={{ width: size, height: size }}>
                <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                    <circle cx={cx} cy={cy} r={radius} fill="none" stroke="var(--border)" strokeWidth={thickness} />
                    {segments.map((seg, i) => (
                        <circle
                            key={i}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <div className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>{total.toFixed(total < 100 ? 1 : 0)}</div>
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-4">
                {segments.map((seg, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-xs">
                        ...
                        <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{(seg.pct * 100).toFixed(0)}%</span>
                    </div>
                ))}
            </div>
        </div>
                ...
                {data.map((d, i) => {
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center gap-1" style={{ width: barWidth }}>
                            {showValues && (
                                <div className="text-[10px] font-semibold" style={{ color: d.color || "var(--accent)" }}>
                            ...
                            ></div>
                            <div className="text-[9px] text-center truncate w-full" style={{ color: "var(--text-muted)" }}>
                                {d.label}
                            </div>
                        </div>
                    );
                    ...
                    return (
                        <div key={i}>
                            <div className="flex justify-between text-xs mb-1">
                                ...
                                <span className="font-semibold ml-2" style={{ color: d.color || "var(--accent)" }}>{d.displayValue || d.value.toFixed(2)}</span>
                            </div>
            ...
            <svg width={size} height={size / 2 + 20} viewBox={`0 0 ${size} ${size / 2 + 20}`}>
                ...
                {pct > 0 && (
                    <path
                        ...
                        fill="none" stroke={color || "var(--accent)"} strokeWidth="12" strokeLinecap="round"
                        style={{ transition: "d 0.6s ease" }}
                    />
                )}
            </svg>
            <div className="text-center -mt-4">
                <div className="text-xl font-bold" style={{ color: color || "var(--accent)" }}>{(pct * 100).toFixed(1)}%</div>
                {label && <div className="text-[10px] mt-1" style={{ color: "var(--text-muted)" }}>{label}</div>}
            </div>
        </div>
    );
}
